...
const toLastNames = people => {
  const comobined = people.map ((item) => {
...
const addValues = (arr) => {
  const summedValues = arr.reduce((previousValue,currentValue) => 
...
const addPurchases = (arr) => {
  const totalPurchased = arr.reduce((previousPurchase, currentPurchase) => {
    return previousPurchase + currentPurchase.purchasePrice;
...
const countNumberOfElements = (arr) => {
  const numOfElements = arr.reduce ((acc, current, index) => {
    acc +current;
    return index+1;
  })
...
const returnNames = (arr) => {
  const names =[]
  arr.reduce((accum , currentNa) => {
    names.push(currentNa.name)
  },0)
  return names;
};
...
const reversedString = (str) => {
  const reversed = str.split('');
   const reversedArr= reversed.reduce((firstCh,secondCh) => {
   return secondCh+firstCh},'' )
   return reversedArr;
};
